...
export async function scanBarcode(barcode: string) {
  ...
  if (gen) {
    const { request, OpenAPI } = gen;
    return request(OpenAPI, { method: 'GET', url: `/scanner/${encodeURIComponent(barcode)}` });
  }
  return baseFetch(`/scanner/${encodeURIComponent(barcode)}`);
}

export async function lowStock(threshold = 5, limit = 20) {
  const gen = await useGeneratedRequest();
  if (gen) {
    const { request, OpenAPI } = gen;
    return request(OpenAPI, { method: 'GET', url: '/inventory/low-stock', query: { threshold, limit } });
  }
  return baseFetch(`/inventory/low-stock?threshold=${threshold}&limit=${limit}`);
}
...
export async function getSku(id: string) {
  ...
  if (gen) {
    const { request, OpenAPI } = gen;
    return request(OpenAPI, { method: 'GET', url: `/inventory/skus/${id}` });
  }
  return baseFetch(`/inventory/skus/${id}`);
}
