import { dispatchAll } from '../api/client'
...
useEffect(() => { (async () => {
  const all = await dispatchAll();
  setTechs(Array.from(new Set(all.map((x:any)=> x.technician?.name).filter(Boolean))).map(n=>({name:n})));
  setJobs(all.map((x:any)=>({ title: x.workOrder?.title, technician: x.technician, start: x.startTime, status: x.workOrder?.status })));
})() }, [])
return (
  <div>
