...
export function LabelPreview({ sku, size, fields }: LabelPreviewProps) {
  ...
  if (!sku) {
    return (
      <div className="flex items-center justify-center h-64 bg-secondary/50 rounded-lg border-2 border-dashed border-border">
        <p className="text-muted-foreground text-center px-4">Select a SKU to preview the label</p>
      </div>
    )
  }

  return (
    <div className="flex items-center justify-center p-8 bg-secondary/50 rounded-lg min-h-64">
      <div
      ...
      >
        ...
        <div className="space-y-2">
          ...
          {fields.includes("category") && <div className="text-xs text-gray-600">{sku.category}</div>}
