...
- **Field Tools**: 22 professional HVAC calculators across various categories, with results savable to work orders. Includes both AI-powered and Manual Cost Estimators.
...
- **Manual Cost Estimator**: Full-featured manual estimation tool for creating detailed cost estimates without AI dependency. Two modes available: (1) Quick Mode for rapid estimates using labor hours, rates, materials, permits, overhead percentage, and profit margin; (2) Comprehensive Mode with detailed line-item tracking supporting labor, materials, equipment, permits, and other categories. Both modes include automatic tax calculation, profit margin application, work order integration, and transactional database safety. Server-side validation ensures comprehensive mode requires at least one line item. Backend API endpoints: POST /api/v1/estimator/manual, PUT /api/v1/estimator/manual/:id, GET /api/v1/estimator/estimates with JWT authentication and full tenant isolation.
- **Proposals System**: Complete proposal generation system for converting estimates into customer-facing proposals. Database models support proposal creation, line items, status tracking (draft, sent, viewed, approved, rejected, expired), customizable terms, and automatic proposal numbering (PROP-YYYY-####). Backend API with endpoints for creating, updating, and managing proposals, plus one-click conversion from estimates. Features professional PDF export capability (jsPDF), customizable templates, and full proposal management UI. Components include ProposalBuilder.tsx, EstimatesList.tsx, and ConvertToProposal.tsx with comprehensive error handling and validation. Backend API endpoints: POST /api/v1/estimator/proposals, GET /api/v1/estimator/proposals/:id, POST /api/v1/estimator/proposals/convert/:estimateId with JWT authentication and full tenant isolation.
