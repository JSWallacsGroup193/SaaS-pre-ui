...
- **Manual Cost Estimator**: Full-featured manual estimation tool for creating detailed cost estimates without AI dependency. Two modes available: (1) Quick Mode for rapid estimates using labor hours, rates, materials, permits, overhead percentage, and profit margin; (2) Comprehensive Mode with detailed line-item tracking supporting labor, materials, equipment, permits, and other categories. Both modes include automatic tax calculation, profit margin application, and work order integration. Backend API endpoints: POST /api/v1/estimator/manual, PUT /api/v1/estimator/manual/:id, GET /api/v1/estimator/estimates with JWT authentication and full tenant isolation.
- **Proposals System**: (IN PROGRESS) Comprehensive proposal generation system for converting estimates into customer-facing proposals. Database models support proposal creation, line items, status tracking (draft, sent, viewed, approved, rejected, expired), customizable terms, and automatic proposal numbering. Backend API ready with endpoints for creating, updating, and managing proposals, plus one-click conversion from estimates. Features PDF export capability (jsPDF), professional templates, and proposal management UI. Backend API endpoints: POST /api/v1/estimator/proposals, GET /api/v1/estimator/proposals/:id, POST /api/v1/estimator/proposals/convert/:estimateId with JWT authentication.
